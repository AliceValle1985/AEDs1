...
{ /* gravar uma matriz real em arquivo. A matriz e o nome do arquivo serão dados como parâmetros. Para testar, usar a leitura da matriz do problema anterior. Usar outro método para ler e recuperar a matriz do arquivo, antes de mostrá-la na tela. */
 ...
 int rows = 0;
 int columns = 0;
 rows = freadIntMatrixRows ( "MATRIX12.TXT" );
 columns = freadIntMatrixColumns ( "MATRIX12.TXT" );
 IO_printf ( "\n" );
 if ( rows <= 0 || columns <= 0 )
 {
     IO_println ( "\nERRO: Valor invalido." );
...
*/
void lermatrixquadrada(int l, int c, int matrix[l][c])
{
    int z = 0;
	chars text = IO_new_chars(STR_SIZE);
    if(l != c)
    {
        printf("ERRO, a matrix deve ser quadrada, digite dois valores iguais.");
    }
    else
    {
        for(int i = 0; i < l; i++)
        {
            ...
            {
                strcpy(text, STR_EMPTY);
                z = IO_readint ( IO_concat (IO_concat ( IO_concat ( text, IO_toString_d ( i ) ), ", " ),IO_concat ( IO_concat ( text, IO_toString_d ( a ) ), " : " ) ) );
                matrix[i][a] = z;
            }		
        }	
    }
...
bool Is0TrianguloCimaMatrix (int linhas, int colunas, double matrix[linhas][colunas])
{
    bool result = 1;
    for (int i = 0; i < linhas; i++)
    {
...
{ /* testar se são zeros os valores acima da diagonal principal de uma matriz real quadrada. */
    ...
    else
    {
        double matrix[linhas][colunas];
        readDoubleMatrix ( linhas, colunas, matrix );
        bool result = Is0TrianguloCimaMatrix(linhas, colunas, matrix);      
        getchar();
        if(result)
        {
            printf("São todos numeros 0");
        ...
        {
            printf("Tem mais numeros alem de 0 acima da diagonal");
        }
        getchar();
        printf("\n");
    }  
}
void ex1a(int linhas, int colunas, int matrix[linhas][colunas])
{
    int valor = 1;
    for(int i = 0; i < linhas; i++)
    {
...
void fprintIntMatrix2 ( chars fileName, int linhas, int colunas, int matrix[linhas][colunas] )
{
 FILE* arquivo = fopen ( fileName, "wt" );
 IO_fprintf ( arquivo, "%d\n", linhas );
 IO_fprintf ( arquivo, "%d\n", colunas );
...
{ 
    ...
    {
        int matrix[linhas][colunas];
        ex1a(linhas, colunas, matrix);
        printIntMatrix ( linhas, colunas, matrix);
        fprintIntMatrix2 ( "Matriz1", linhas, colunas, matrix);
        getchar();
        printf("\n");
    }
}

void ex2a(int linhas, int colunas, int matrix[linhas][colunas])
{
    int valor = 1;
    for(int i = linhas-1; i >= 0; i--)
    {
...
void ex2()
{
    ...
    {
        int matrix[linhas][colunas];
        ex2a(linhas, colunas, matrix);
        printIntMatrix ( linhas, colunas, matrix);
        fprintIntMatrix2 ( "Matriz2", linhas, colunas, matrix);
        getchar();
        printf("\n");
    }
}
...
int main(void)
{
    ...
    do
    {
        ...
        printf ( "\n%s" , "19 - Method_19" );
        printf ( "\n%s" , "20 - Method_20" );
        IO_println ( "21 - Exercicio extra 1");
        IO_println ( "22 - Exercicio extra 2");
        printf ( "\n" );
        printf ( "\n%s", "Opcao = " );
        scanf ( "%d", &x );
        getchar( );
