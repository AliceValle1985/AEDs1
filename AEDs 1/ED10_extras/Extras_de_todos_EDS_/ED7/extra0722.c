//Alice Pereira Valle Guimaraes - 813338 - 08/10/2023
#include<stdio.h>
#include "io.h"
#include <math.h>
...
void trem2(chars filename)
{
    ...
    {
        resultado = trem1(i);
        fprintf(arquivo, "Quantidade: %d     Resultado: %.2lf\n", i, resultado);
        printf("Quantidade: %d     Resultado: %.2lf\n", i, resultado);
    }
    fclose(arquivo);
...
void method16()
{
    ...
    chars nomearquivo = "RESULTADO06.txt";
    trem2(nomearquivo);
    printf("Aperte ENTER para continuar");
}//end method16

void trem17(chars filename)
{
    FILE *arquivo = fopen(filename, "w");
    int x         = 0;
    double potencia  = 1;
    ...
    printf("Soma = %.10lf", soma);
    fprintf(arquivo, "Soma = %.10lf", soma);
    fclose(arquivo);
}
void method17()
{
    IO_id ( "Method_17 - v0.0" );
    chars nomearquivo = "RESULTADO07.txt";
    trem17(nomearquivo);
    printf("Aperte ENTER para continuar");
}//end method17
...
void method18()
{
    IO_id ( "\nMethod_18 - v0.0" );
    chars nomearquivo = IO_new_chars(STR_SIZE);
    printf("De o nome para o arquivo: ");
    scanf("%s", nomearquivo);
    method18a(nomearquivo);
    printf("Aperte ENTER para continuar");
}//end method18
void method19a(chars filename1, chars filename2)
{
    ...
    while(fgets(palavras, STR_SIZE, arquivo1) != NULL)
    {
...
void method19()
{
    chars nomearquivo1 = IO_new_chars(STR_SIZE);
    chars nomearquivo2 = "RESULTADO09.TXT";
    printf("\nNome do arquivo que sera lido: ");
    scanf("%s", nomearquivo1);
    method19a(nomearquivo1, nomearquivo2);
}//end method19

void method20a(chars filename1, chars filename2)
{
    FILE *arquivo1 = fopen(filename1, "r");
    FILE *arquivo2 = fopen(filename2, "w");
    if (arquivo1 == NULL || arquivo2 == NULL)
...
{
    chars nomearquivo1 = IO_new_chars(STR_SIZE);
    chars nomearquivo2 = "RESULTADO10.TXT";
    printf("\nNome do arquivo que sera lido: ");
    scanf("%s", nomearquivo1);
    method20a(nomearquivo1, nomearquivo2);
}
void valorint(chars filename)
{
    FILE *arquivo = fopen(filename, "w");
    if(arquivo == NULL)
    {
...
void method21()
{
    chars arquivonome = IO_new_chars(STR_SIZE);
    printf("\nDigite o nome do arquivo: ");
    scanf("%s", arquivonome);
    valorint(arquivonome);
}
void depoisd(chars filename)
{
    int d = 0;
    ...
    while(! feof ( arquivo ))
    {
        ...
        {
            fscanf(arquivo, "%c", &palavra[i]);
            if(palavra[0] == 'd' || palavra[0] == 'D');
            {
                printf("%d\n", d);
               d++; 
            }
            if(palavra[i] == ' ' && palavra[i + 1] == 'd' ||  palavra[i + 1] == 'D');
            {
                printf("%d\n", d);
               d++; 
            }
    ...
    printf("\nContagem de palavras com d é igual a: %d", d);
}
void method22()
{
    chars arquivonome = IO_new_chars(STR_SIZE);
