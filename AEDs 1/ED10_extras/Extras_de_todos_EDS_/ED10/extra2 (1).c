...
void matrixTranspose(int linhas, int colunas, int matrix1[linhas][colunas], int matrix2[linhas][colunas])
{
...
{
    int linhas, colunas;
    FILE* arquivo = fopen("matrix1.txt", "r");
    fscanf(arquivo, "%d %d", &linhas, &colunas);  
    int matrix1[linhas][colunas];
    int matrix2[linhas][colunas];
    freadintmatrix("matrix1.txt");
    matrixTranspose(linhas, colunas, matrix1, matrix2);
    fclose(arquivo);
}
...
bool matrixzero()
{
    ...
    ref_int_matrix matrix = freadintmatrix("DADOS.TXT");
    int linhas = 0;
    int colunas = 0;
    if(matrix)
    {
...
void method17()
{
    bool resposta = matrixzero();
    if(resposta == true)
    {
        printf("\nTodo os valores são igual a zero.");
    }
    else
    {
        printf("\nTem um valor que não é igual a zero.");
    }
}
...
bool testarigual()
{
    bool resposta = true;
    ref_int_matrix matrix = freadintmatrix("DADOS.TXT");
    ref_int_matrix matrix2 = freadintmatrix("DADOS2.TXT");
    if(matrix && matrix2)
    {
        for (matrix ->ix = 0; matrix->ix < matrix->linhas; matrix->ix = matrix->ix + 1 )
        {
            for ( matrix->iy = 0; matrix->iy < matrix->colunas; matrix->iy = matrix->iy + 1 )
            {
                if(matrix->data[matrix->ix][matrix->iy] != matrix2->data[matrix->ix][matrix->iy])
                {
                     resposta = false;
                }
            }
...
int matrixadd(ref_int_matrix matrix, int n, ref_int_matrix matrix2)
{
    int soma = 0;
    //soma
    for (matrix2 ->ix = 0; matrix2->ix < matrix2->linhas; matrix2->ix = matrix2->ix + 1 )
    {
        for ( matrix2->iy = 0; matrix2->iy < matrix2->colunas; matrix2->iy = matrix2->iy + 1 )
        {
            soma = matrix2->data[matrix2 ->ix][matrix2 ->iy] * n + soma;
            soma = matrix->data[matrix2 ->ix][matrix2 ->iy] + soma;
        }
    }
    return (soma);
}
void method19()
{
    ...
    printf("\nDigite um numero: ");
    scanf("%d", &n);
    soma = matrixadd(matrix, n, matrix2);
    printf("\nSoma das matrizes = %d", soma);
}
...
int matrixproduct(ref_int_matrix matrix, ref_int_matrix matrix2)
{
    int produto = 0;
    int produto2 = 0;
    for (matrix2 ->ix = 0; matrix2->ix < matrix2->linhas; matrix2->ix = matrix2->ix + 1 )
    {
        for ( matrix2->iy = 0; matrix2->iy < matrix2->colunas; matrix2->iy = matrix2->iy + 1 )
        {
            produto = matrix2->data[matrix2 ->ix][matrix2 ->iy] * matrix->data[matrix2 ->ix][matrix2 ->iy];
            produto2 += produto;
        }
    }
    return (produto2);
...
void method20()
{
    ...
    ref_int_matrix matrix = freadintmatrix("DADOS.TXT");
    ref_int_matrix matrix2 = freadintmatrix("DADOS2.TXT");
    produto = matrixproduct(matrix, matrix2);
    printf("\nProduto das matrizes = %d", produto);
}
/*colocar um arranjo em ordem crescente, pelo método de trocas de posição.
Para testar, receber um nome de arquivo como parâmetro e 
aplicar a função sobre o arranjo com os valores lidos.
Exemplo: arranjo1 = readArrayFromFile ( "DADOS1.TXT" );
ordenado = sortArrayUp ( arranjo );
*/

int_array sortarray(int_array array)
{
    array = readarrayfromfile3("DADOS.TXT");
    int temp = 0;
    if(array.data)
    {
        for (array.ix = 0; array.ix < array.length - 1; array.ix++)
        {
            for (int j = 0; j < array.length - array.ix - 1; j++)
            {
...
ref_int_matrix alloc_int_matrix(int linhas, int colunas) {
    ref_int_matrix matrix = (ref_int_matrix)malloc(sizeof(int_matrix));
    matrix->linhas = linhas;
    matrix->colunas = colunas;
    matrix->data = (int **)malloc(linhas * sizeof(int *));
    for (int i = 0; i < linhas; i++) {
        matrix->data[i] = (int *)malloc(colunas * sizeof(int));
    }
    return matrix;
}
ref_int_matrix retornavalorduasmatrix(ref_int_matrix matrix, ref_int_matrix matrix2)
{
    ref_int_matrix matrix3 = alloc_int_matrix(matrix->linhas, matrix2->colunas);
    for (int i = 0; i < matrix->linhas; i++)
    {
        for (int j = 0; j < matrix2->colunas; j++)
        {
            for (int k = 0; k < matrix->colunas; k++)
            {
                matrix3->data[i][j] += matrix->data[i][k] * matrix2->data[k][j];
            }
        }
    }
    return matrix3;
...
int identitymatrix()
{
    ref_int_matrix matrix = freadintmatrix("DADOS.TXT");
    ref_int_matrix matrix2 = freadintmatrix("DADOS2.TXT");
    ref_int_matrix valordasduasmatrix = retornavalorduasmatrix(matrix, matrix2);
    
    
    ref_int_matrix matrixidentidade = freadintmatrix("DADOS2.TXT");
    for (matrix2 ->ix = 0; matrix2->ix < matrix2->linhas; matrix2->ix = matrix2->ix + 1 )
    {
        for ( matrix2->iy = 0; matrix2->iy < matrix2->colunas; matrix2->iy = matrix2->iy + 1 )
        {
            if(valordasduasmatrix->data == matrixidentidade->data)
            {
                printf("\nAs matrizes são iguais.");
            }
        }
