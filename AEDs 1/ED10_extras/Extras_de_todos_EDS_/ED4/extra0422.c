...
Exemplo: n = 10 e [ 14.2 : 23.1 ] com { 5.1, 10.5, 12.4, 14.2, 15.3, 18.3, 20.4, 21.7, 23.1, 24.2 }*/
    float menor  = 0.0;
    float maior  = 0.0;
    float numero = 0.0;
    //repeticao para sempre ter o menor e o maior numero
    do
    {
        //pedindo e recebendo valores
    ...
    scanf("%f", &numero);
    //testando se esta entre os dois numeros dados
    if(menor < numero && numero < maior)
    {
         printf("O numero %.2f esta entre o intervalo de %.2f %.2f.", numero, menor, maior);
    }
    else
    {
        printf("O numero %.2f nao esta entre o intervalo de %.2f %.2f.", numero, menor, maior);
    }
}
bool maiork(char string)
{
...
{
    /* ler uma sequência de caracteres do teclado;
- mostrar a quantidade de letras maiúsculas maiores que 'K' contadas por uma função 
...
Exemplo: sequência = AaKkLmM0*Nx*/
    ...
    printf("\nQuantidade de maiores que K: %d", maiorka);
}

int method_14()
{
    /*- ler uma sequência de caracteres do teclado;
- mostrar as letras maiúsculas maiores que 'K' separadas por uma função 
 definida para receber uma cadeia de caracteres como parâmetro*/
    char palavra[STR_SIZE];
    int tamanho = 0;
    printf("\nDigite uma palavra: ");
...
bool func15(char string)
{
    bool resultado = false;
    if((string >= 'k'  && string <= 'z') || (string >= 'K'  && string <= 'Z'))
    {
         resultado = true;
    }
    return resultado;
...
{
    /*- ler uma sequência de caracteres do teclado;
- mostrar a quantidade de letras (tanto maiúsculas, quanto minúsculas) maiores que 'K' e 'k'
 contadas por uma função definida para receber uma cadeia de caracteres como parâmetro.*/
    ...
    printf("\nQuantidade de maiores que k e K: %d", maiork);
}
int method_16()
{
    char palavra[STR_SIZE];
...
bool digitimpar(char string)
{
    bool resultado = false;
    if(string == '1' || string == '3' || string == '5' || string == '7' || string == '9')
    {
        resultado = true;
    }
    return (resultado);
...
- mostrar a quantidade de dígitos ímpares em uma cadeia de caracteres contados por uma função
 ...
 DICA: Considerar o valor inteiro do código equivalente (type casting) para teste. */
    ...
    for(int i = 0; i < tamanho; i++)
    {
        if(digitimpar (palavra[i]))
        {
            printf("Digitos impares: %c\n ", palavra[i]);
            contagem++;
        }
    }
    printf("\nQuantidade de impares: %d", contagem);
}
bool naoalfanumerico(char string)
{
...
*/
    ...
    printf("\nQuantidade de alfanumericos: %d", contagem);
}

int contalfasim(char strings[][STR_SIZE], int numberOfStrings) {
    int totalSymbols = 0;
    for (int i = 0; i < numberOfStrings; i++) {
        int symbolsInString = 0;
        for (int j = 0; j < strlen(strings[i]); j++) {
            if (isalnum(strings[i][j]) || isspace(strings[i][j])) {
                symbolsInString++;
            }
        }
        printf("\nQuantidade de símbolos alfanuméricos na palavra %d: %d\n", i + 1, symbolsInString);
        totalSymbols += symbolsInString;
    }
    return totalSymbols;
}
...
void method_21() {
    int numberOfStrings;
    printf("\nDigite a quantidade de palavras a serem inseridas: ");
    scanf("%d", &numberOfStrings);

    char strings[numberOfStrings][STR_SIZE];
    ...
    int total = contalfasim(strings, numberOfStrings);
    printf("\nTotal de símbolos alfanuméricos: %d", total);
}
...
void method_22()
{
    int cont1 = 0;
    int cont2 = 0;
    char palavra1[STR_SIZE];
    char palavra2[STR_SIZE];
    ...
    {
        if(palavra1[i + 1] >= '0' && palavra1[i + 1] <= '9')
        {
            cont1++;
        }
    }
    ...
    {
        if(palavra2[i + 1] >= '0' && palavra2[i + 1] <= '9')
        {
            cont2++;
        }
    }
    if(cont1 > cont2)
    {
        printf("\nA primeira string tem mais digitos.\nDigitos: %d", cont1);
    }
    if(cont1 < cont2)
    {
        printf("\nA segunda string tem mais digitos.\nDigitos: %d", cont2);
    }
}
int main (void)
{
